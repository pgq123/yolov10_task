...
def draw_bbox(img, i, j, big_bboxs, windows):
...
def make_classes(c_name):
    f = open('classes.txt', 'r+')
    txt = [r.rsplit(': ', 1)[1] for r in f.readlines()]
    id = len(txt)
    in_ = str(id) + ': ' + c_name + '\n'
    if (c_name + '\n') not in txt:
        f.write(in_)
...
def make_txt(df, i, j, big_bboxs, windows, file1, file2, out_dir):
    if i >= cell_num // 2:
        i -= cell_num // 2
    f = open('all_classes.txt', 'r')
    c_name = df.iloc[i, 9]
    label_dict = dict(zip([0, 1, 2, 3, 4, 5, 6, 7, 8], ['cell', 'L', 'C', 'R', 'Cr', 'Li', 'cell', 'cell', 'cell']))
    txt = [r.split(' ') for r in f.readlines()]
    f.close()
    for im, t in enumerate(txt):
        if c_name in t:
            id_, c_name = im, label_dict[im]
    if c_name == 'cell':
        id_ = 0
    big_top_left = np.clip(((big_bboxs[i, :2] - windows[j, :2]) * 0.25).astype(int), 0, 1024)
    big_bottom_right = np.clip(((big_bboxs[i, 4:6] - windows[j, :2]) * 0.25).astype(int), 0, 1024)
    c_x, c_y = (big_bottom_right - big_top_left) // 2 + big_top_left
    width = big_bottom_right[0] - big_top_left[0]
    height = big_bottom_right[1] - big_top_left[1]
    if file1 in ['PWYL', 'PWYN-C', 'PWYN-S', 'PWZF-C'] or (file1 == 'PWZF-S' and file2 == 'CS'):
        outdir_l = f'datasets/labels_test/train/{file1}_{file2}_{j}.txt'
    else:
        outdir_l = f'datasets/labels_test/val/{file1}_{file2}_{j}.txt'
    scale = 1.0 / 1024
    if width / 1024 == 0.0 or height / 1024 == 0.0:
        return
    fi = open(outdir_l, 'a')
    # sa, sb = 'Image', '/Data/'
    # in_ = out_dir + os.sep + file1 + '_' + file2 + '_' + str(j) + '.jpg'
    # out = sb.join(in_.rsplit(sa, 1)).rsplit(".", 1)[0] + ".jpg"
    # if not os.path.exists(out):
    #     shutil.copy(f'{out_dir}/{file1}_{file2}_{j}.jpg', out)
    msg = str(id_) + ' ' + str(c_x * scale) + ' ' + str(c_y * scale) + ' ' + str(width * scale) + ' ' + str(height * scale)
    fi.write(msg + '\n')
    fi.close()
    # make_classes(c_name)
    ...
    for file1 in file1_name:
        for file2 in file2_name:
            ...
            for i in range(cell_num):
                for j in point[i]:
                    make_txt(df, i, j, big_bbox, windows, file1, file2, out_dir)
